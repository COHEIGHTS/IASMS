...
$programmes = array("-", "Computer Science", "Education", "Medicine and Surgery", "Veterinary Medicine", "Actuarial Science", "Electrical Engineering",
    "Mechanical Engineering", "Gender", "Biochemistry", "Econ&Stat", "Animal Science");

$get_user_details = "SELECT * FROM internal_registration WHERE index_number='$student_index' AND first_name='$student_fname' AND last_name='$student_lname'";
...
if ($execute_get_details_command == 1) {
    ...
    setcookie("student_registration_type", "internal registration", time() + (86400 * 30));
    $student_registration_type = "INTERNAL REGISTRATION";
        ...
        if ($_POST["txt_company_name"] != "" && $_POST["txt_supervisors_name"] != "" && $_POST["txt_supervisors_contact"] != "" && $_POST["txt_supervisors_email"] != "" && $_POST["selected_region"] != "" && $_POST["txt_address"] != "") {
            ...
            if ($check_avoidance_query == 1) {
                ...
                if ($run_query = mysqli_query($conn, $my_insert_query)) {
                    ...
                    if ($student_registration_type == "INTERNAL REGISTRATION") {
                        $my_update_query = "UPDATE `internal_registration` SET `company_supervisor_name` = '$student_supervisor_name' WHERE index_number = '$student_index_number'";
                        $execute_my_update_query = mysqli_query($conn, $my_update_query);
                        
                        $my_update_query2 = "UPDATE `internal_registration` SET `company_supervisor_contact` = '$student_supervisor_contact' WHERE index_number = '$student_index_number'";
                        $execute_my_update_query = mysqli_query($conn, $my_update_query2);
                        
                        $my_update_query3 = "UPDATE `internal_registration` SET `attachment_region` = '$student_company_region' WHERE index_number = '$student_index_number'";
