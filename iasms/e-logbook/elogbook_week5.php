...
$stmt = $conn->prepare("SELECT * FROM week5_table WHERE index_number = ?");
...
if (isset($_POST["btn_save"]) || isset($_POST["btn_update"])) {
    ...
    if (!empty($monday_job_assigned) && !empty($monday_skill_acquired) &&
        ...
        if (isset($_POST["btn_save"]) && !$is_existing_data) {
            // Insert new data
            $stmt = $conn->prepare("INSERT INTO week5_table (username, index_number, monday_job_assigned, monday_special_skill_acquired, tuesday_job_assigned, tuesday_special_skill_acquired, wednesday_job_assigned, wednesday_special_skill_acquired, thursday_job_assigned, thursday_special_skill_acquired, friday_job_assigned, friday_special_skill_acquired, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
    ...
    <div class="container-fluid">
      <div class="panel">
        <div class="panel-heading phead">
          <h2 class="panel-title ptitle"> E-LogBook-Week 5</h2>
        </div>
        <div class="panel-body pbody">
          <div id="navigation_holder">
            <ul class="pager">
              <li class="previous"><a href="elogbook_week4.php">← Previous</a></li>
              <li class="next"><a href="elogbook_week6.php">Next →</a></li>
            </ul>
          </div>
          <div id="week_holder">
            ...
            <table class="table table-bordered">
              <thead>
                <tr>
                  ...
                  <th style="text-align:center">Job Assigned</th>
              ...
              <tbody>
                ...
                $job_assigned_fields = array("job_assigned_1", "job_assigned_2", "job_assigned_3", "job_assigned_4", "job_assigned_5");
                $skill_acquired_fields = array("skill_acquired_1", "skill_acquired_2", "skill_acquired_3", "skill_acquired_4", "skill_acquired_5");

                // Use holders instead of querying again
                $holders = [
                    "monday" => [$monday_job_assigned_holder, $monday_skill_acquired_holder],
                    "tuesday" => [$tuesday_job_assigned_holder, $tuesday_skill_acquired_holder],
                    "wednesday" => [$wednesday_job_assigned_holder, $wednesday_skill_acquired_holder],
                    "thursday" => [$thursday_job_assigned_holder, $thursday_skill_acquired_holder],
                    "friday" => [$friday_job_assigned_holder, $friday_skill_acquired_holder]
                ];

                for ($i = 0; $i < 5; $i++) {
                    $day_lower = strtolower($days[$i]);
                    echo '<tr>
                            <td style="text-align:center">' . $days[$i] . '</td>
                            <td><textarea class="form-control" name="' . $job_assigned_fields[$i] . '" placeholder="Write Job Assigned">' . $holders[$day_lower][0] . '</textarea></td>
                            <td><textarea class="form-control" name="' . $skill_acquired_fields[$i] . '" placeholder="Write Skills Acquired">' . $holders[$day_lower][1] . '</textarea></td>
                          </tr>';
                }
                ?>
              </tbody>
