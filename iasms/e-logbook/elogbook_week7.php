...
$stmt = $conn->prepare("SELECT * FROM week7_table WHERE index_number = ?");
...
if (isset($_POST["btn_save"]) || isset($_POST["btn_update"])) {
    ...
    if (!empty($monday_job_assigned) && !empty($monday_skill_acquired) &&
        ...
        if (isset($_POST["btn_save"]) && !$is_existing_data) {
            // Insert new data
            $stmt = $conn->prepare("INSERT INTO week7_table (username, index_number, monday_job_assigned, monday_special_skill_acquired, tuesday_job_assigned, tuesday_special_skill_acquired, wednesday_job_assigned, wednesday_special_skill_acquired, thursday_job_assigned, thursday_special_skill_acquired, friday_job_assigned, friday_special_skill_acquired, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
    ...
    <div class="container-fluid">
      <div class="panel">
        <div class="panel-heading phead">
          <h2 class="panel-title ptitle"> E-LogBook-Week 7</h2>
        </div>
        <div class="panel-body pbody">
          <div id="navigation_holder">
            <ul class="pager">
              <li class="previous"><a href="elogbook_week6.php">← Previous</a></li>
              <li class="next"><a href="elogbook_week8.php">Next →</a></li>
            </ul>
          </div>
          <div id="week_holder">
