...
$stmt = $conn->prepare("SELECT * FROM week3_table WHERE index_number = ?");
...
if ($is_existing_data) {
    ...
    $monday_job_assigned_holder = $existing_data['monday_job_assigned'];
    $monday_skill_acquired_holder = $existing_data['monday_special_skill_acquired'];
    $tuesday_job_assigned_holder = $existing_data['tuesday_job_assigned'];
    $tuesday_skill_acquired_holder = $existing_data['tuesday_special_skill_acquired'];
    $wednesday_job_assigned_holder = $existing_data['wednesday_job_assigned'];
    $wednesday_skill_acquired_holder = $existing_data['wednesday_special_skill_acquired'];
    $thursday_job_assigned_holder = $existing_data['thursday_job_assigned'];
    $thursday_skill_acquired_holder = $existing_data['thursday_special_skill_acquired'];
    $friday_job_assigned_holder = $existing_data['friday_job_assigned'];
    $friday_skill_acquired_holder = $existing_data['friday_special_skill_acquired'];
}
...
if (isset($_POST["btn_save"]) || isset($_POST["btn_update"])) {
    ...
    if (!empty($monday_job_assigned) && !empty($monday_skill_acquired) &&
        ...
        if (isset($_POST["btn_save"]) && !$is_existing_data) {
            // Insert new data
            $stmt = $conn->prepare("INSERT INTO week3_table (username, index_number, monday_job_assigned, monday_special_skill_acquired, tuesday_job_assigned, tuesday_special_skill_acquired, wednesday_job_assigned, wednesday_special_skill_acquired, thursday_job_assigned, thursday_special_skill_acquired, friday_job_assigned, friday_special_skill_acquired, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            ...
            if ($stmt->execute()) {
                ...
                $monday_job_assigned_holder = $monday_job_assigned;
                $monday_skill_acquired_holder = $monday_skill_acquired;
                $tuesday_job_assigned_holder = $tuesday_job_assigned;
                $tuesday_skill_acquired_holder = $tuesday_skill_acquired;
                $wednesday_job_assigned_holder = $wednesday_job_assigned;
                $wednesday_skill_acquired_holder = $wednesday_skill_acquired;
                $thursday_job_assigned_holder = $thursday_job_assigned;
                $thursday_skill_acquired_holder = $thursday_skill_acquired;
                $friday_job_assigned_holder = $friday_job_assigned;
                $friday_skill_acquired_holder = $friday_skill_acquired;
            } else {
                echo "<script>alert('Error saving data: " . addslashes($conn->error) . "');</script>";
            }
        } elseif (isset($_POST["btn_update"]) && $is_existing_data) {
            // Update existing data
            $stmt = $conn->prepare("UPDATE week3_table SET monday_job_assigned = ?, monday_special_skill_acquired = ?, tuesday_job_assigned = ?, tuesday_special_skill_acquired = ?, wednesday_job_assigned = ?, wednesday_special_skill_acquired = ?, thursday_job_assigned = ?, thursday_special_skill_acquired = ?, friday_job_assigned = ?, friday_special_skill_acquired = ? WHERE index_number = ?");
...
<body>
  ...
  <div id="top-navigation">
    ...
    <div id="student_name"><span style="color:rgb(255, 198, 0);font-size:1.1em"><em>Welcome,</em>  </span><span
        style="font-family:serif"><?php echo htmlspecialchars($student_full_name); ?></span></div>
  </div>
    ...
    <div class="container-fluid">
      <div class="panel">
        <div class="panel-heading phead">
          <h2 class="panel-title ptitle"> E-LogBook- Week 3</h2>
        </div>
        <div class="panel-body pbody">
          <div id="navigation_holder">
            <ul class="pager">
              <li class="previous"><a href="elogbook_week2.php">← Previous</a></li>
              <li class="next"><a href="elogbook_week4.php">Next →</a></li>
            </ul>
          </div>
          <div id="week_holder">
              ...
              <tbody>
                <tr>
                  ...
                  <td><textarea name='job_assigned_1'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($monday_job_assigned_holder); ?></textarea>
                  </td>
                  <td><textarea name='skill_acquired_1'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($monday_skill_acquired_holder); ?></textarea>
                  </td>
                </tr>
                <tr>
                  <td style='padding:20px;text-align:center'>Tuesday</td>
                  <td><textarea name='job_assigned_2'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($tuesday_job_assigned_holder); ?></textarea>
                  </td>
                  <td><textarea name='skill_acquired_2'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($tuesday_skill_acquired_holder); ?></textarea>
                  </td>
                </tr>
                <tr>
                  <td style='padding:20px;text-align:center'>Wednesday</td>
                  <td><textarea name='job_assigned_3'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($wednesday_job_assigned_holder); ?></textarea>
                  </td>
                  <td><textarea name='skill_acquired_3'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($wednesday_skill_acquired_holder); ?></textarea>
                  </td>
                </tr>
                <tr>
                  <td style='padding:20px;text-align:center'>Thursday</td>
                  <td><textarea name='job_assigned_4'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($thursday_job_assigned_holder); ?></textarea>
                  </td>
                  <td><textarea name='skill_acquired_4'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($thursday_skill_acquired_holder); ?></textarea>
                  </td>
                </tr>
                <tr>
                  <td style='padding:20px;text-align:center'>Friday</td>
                  <td><textarea name='job_assigned_5'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($friday_job_assigned_holder); ?></textarea>
                  </td>
                  <td><textarea name='skill_acquired_5'
                      class='form-control adjusted_text_area'><?php echo htmlspecialchars($friday_skill_acquired_holder); ?></textarea>
                  </td>
                </tr>
              </tbody>
            </table>
