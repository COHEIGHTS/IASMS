...
if (isset($_POST["btn_login"])) {
    if ($_POST["txtvisiting_supervisor_password"] != "") {
        ...
        $supervisor_login_details = "SELECT * FROM supervisors_login WHERE status = 'Visiting' AND username = ?";
        ...
        if ($result->num_rows == 1) {
            ...
            if (password_verify($supervisors_password, $row['password'])) {
                // Successful login, redirect to visiting supervisor grade page
                header("Location: visiting_supervisor_grade.php");
                exit();
            } else {
                echo "<script>alert('Invalid credentials or you are not a Visiting Supervisor.');</script>";
            }
        } else {
  ...
  <div id="left_side_bar">
    <ul id="menu_list">
      ...
      <a class="menu_items_link" href="visiting_supervisor_login.php">
        <li class="menu_items_list" style="background-color:orange;padding-left:16px">Visiting Supervisor</li>
      </a>
